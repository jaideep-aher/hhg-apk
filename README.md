...
| **Version** | `Version 8` (versionCode `8`) — matches **`ready-to-use/version-8.apk`**. **Architecture**: the app is a Kotlin "shell" that hosts pages from the Next.js site (`android/webapp/`, deployed separately — temporarily at a staging host, set via `WEB_BASE_URL`) inside a WebView. The shell keeps native responsibility for **force-update gating** (runs before any webview), navigation, location/permissions, push (FCM), farmer-ID storage, offline cache, and in-app updates; all content screens (home, farmer dashboard, Hundekari rates, AI trends, seeds, Local Vyapar, About) are webpages. This lets you ship a new feature by pushing to the webapp — no APK release required. The base URL and drawer menu items are also remote-configurable via `GET /api/config` (`webBaseUrl`, `webBaseUrlFallback`, `menuItems`). WebView load failures surface localized Marathi errors via `NetworkErrors` — backend hostnames are never exposed in any error message. Previous **`version-5.apk`** through **`version-1.apk`** are kept in the same folder for history / rollback. |
